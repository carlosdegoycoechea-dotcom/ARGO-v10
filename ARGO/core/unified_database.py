...
class UnifiedDatabase:
    ...
    def create_project(
        ...
        with self._get_connection() as conn:
            ...
            logger.info(f"Proyecto creado: {name} (ID: {project_id}, Tipo: {project_type})")
            return project_id
    ...
    def register_file(
        ...
        with self._get_connection() as conn:
            ...
            logger.debug(f"Archivo registrado: {filename} (Proyecto: {project_id})")
    ...
    def save_conversation(
        ...
        with self._get_connection() as conn:
            ...
            logger.debug(f"Conversación guardada (Sesión: {session_id}, Mensajes: {len(messages)})")
    ...
    def save_analysis(self, project_id: str, analysis_type: str, query: str,
        ...
        with self._get_connection() as conn:
            ...
            logger.debug(f"Análisis guardado (Tipo: {analysis_type}, Confianza: {confidence})")
    ...
    def insert_api_usage(
        ...
        with self._get_connection() as conn:
            ...
            logger.debug(
                f"API usage recorded: {provider}/{model} - {total_tokens} tokens, ${round(cost_estimated, 4)}"
            )
